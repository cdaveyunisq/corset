...
ReadList *read_salmon_eq_classes_file(string all_file_names, TranscriptList *trans, int sample)
{
  ...
  while (getline(ss, filename, ','))
  {
    ...
    std::cout << "Reading data on " << ntranscripts << " transcripts in " << neqclasses << " equivalence classes" << std::endl;
...
void print_usage()
{
  ...
  std::cout << "\t The input files should be multi-mapped bam files. They can be single, paired-end or mixed" << std::endl;
  ...
  std::cout << std::endl;
  std::cout << "\t If you want to combine the results from different transcriptomes. i.e. the same reads have " << std::endl;
  std::cout << "\t been mapped twice or more, you can used a comma separated list like below:" << std::endl;
  std::cout << "\t corset sample1_Trinity.bam,sample1_Oases.bam sample2_Trinity.bam,sample2_Oases.bam ..." << std::endl;
  std::cout << std::endl;
  std::cout << "Options are:" << std::endl;
  std::cout << std::endl;
  std::cout << "\t -d <double list> A comma separated list of distance thresholds. The range must be" << std::endl;
  std::cout << "\t                  between 0 and 1. e.g -d 0.4,0.5. If more than one distance threshold" << std::endl;
  ...
  std::cout << std::endl;
  std::cout << "\t -D <double>      The value used for thresholding the log likelihood ratio. The default " << std::endl;
  std::cout << "\t                  value will depend on the number of degrees of freedom (which is the " << std::endl;
  std::cout << "\t                  number of groups -1). By default D = 17.5 + 2.5 * ndf, which corresponds " << std::endl;
  std::cout << "\t                  approximately to a p-value threshold of 10^-5, when there are fewer than" << std::endl;
  std::cout << "\t                  10 groups." << std::endl;
  std::cout << std::endl;
  std::cout << "\t -I               Switches off the log likelihood ratio test and should be used " << std::endl;
  std::cout << "\t                  for downstream differential transcript usage analysis. It will prevent " << std::endl;
  std::cout << "\t                  differentially expressed transcript being split into different clusters. " << std::endl;
  std::cout << "\t                  This option is the equivalent of setting -D to a very large number.  " << std::endl;
  std::cout << "\t                  Default: log likelihood ratio test on" << std::endl;
  std::cout << std::endl;
  std::cout << "\t -m <int>         Filter out any transcripts with fewer than this many reads aligning." << std::endl;
  std::cout << "\t                  Default: 10" << std::endl;
  std::cout << std::endl;
  std::cout << "\t -g <list>        Specifies the grouping. i.e. which samples belong to which experimental" << std::endl;
  std::cout << "\t                  groups. The parameter must be a comma separated list (no spaces), with the " << std::endl;
  std::cout << "\t                  groupings given in the same order as the bam filename. For example:" << std::endl;
  std::cout << "\t                  -g Group1,Group1,Group2,Group2 etc. If this option is not used, each sample" << std::endl;
  std::cout << "\t                  is treated as an independent experimental group." << std::endl;
  std::cout << std::endl;
  std::cout << "\t -p <string>      Prefix for the output filenames. The output files will be of the form" << std::endl;
  std::cout << "\t                  <prefix>-counts.txt and <prefix>-clusters.txt. Default filenames are:" << std::endl;
  std::cout << "\t                  counts.txt and clusters.txt" << std::endl;
  std::cout << std::endl;
  std::cout << "\t -f <true/false>  Specifies whether the output files should be overwritten if they already exist." << std::endl;
  std::cout << "\t                  Default: false" << std::endl;
  std::cout << std::endl;
  std::cout << "\t -n <string list> Specifies the sample names to be used in the header of the output count file." << std::endl;
  ...
  std::cout << "\t                  Output a file summarising the read alignments. This may be used if you" << std::endl;
  std::cout << "\t                  would like to read the bam files and run the clustering in seperate runs" << std::endl;
  std::cout << "\t                  of corset. e.g. to read input bam files in parallel. The output will be the" << std::endl;
  std::cout << "\t                  bam filename appended with " << corset_extension << "." << std::endl;
  std::cout << "\t                  Default: false" << std::endl;
  std::cout << std::endl;
  std::cout << "\t -i <bam/corset/salmon_eq_classes>  The input file type. Use -i corset, if you previously ran" << std::endl;
  std::cout << "\t                  corset with the -r option and would like to restart using those" << std::endl;
  std::cout << "\t                  read summary files. Use salmon_eq_classes, if you aligned with salmon with" << std::endl;
  std::cout << "\t                  the flag --dumpEq and are passing corset the equivalent class files." << std::endl;
  std::cout << "\t                  Running with either -i corset or salmon_eq_classes will switch off the -r option." << std::endl;
  std::cout << "\t                  Default: bam" << std::endl;
  std::cout << std::endl;
  std::cout << "\t -l <int>         If running with -i corset or salmon_eq_classes, this will filter out a link between contigs" << std::endl;
  std::cout << "\t                  if the link is supported by less than this many reads (performed sample-wise). Reads will " << std::endl;
  std::cout << "\t                  be reassigned uniformly to the contigs in the equivalence class. This option will" << std::endl;
  std::cout << "\t                  improve runtime and memory usage, but will increase the number of clusters reported." << std::endl;
  std::cout << "\t                  Default: 1 (no filtering)" << std::endl;
  std::cout << std::endl;
  std::cout << "\t -x <int>         If running with -i corset or salmon_eq_classes, this option will filter out reads that" << std::endl;
  std::cout << "\t                  align to more than x contigs. Default: no filtering" << std::endl;
  std::cout << std::endl;
  std::cout << "Citation: Nadia M. Davidson and Alicia Oshlack, Corset: enabling differential gene expression " << std::endl;
...
int main(int argc, char **argv)
{
  ...
  while ((c = getopt(argc, argv, "f:p:d:n:g:D:Im:r:i:l:x:")) != EOF)
  {
    ...
    case 'I':
    {
      std::cout << "Switching likelihood test off  - will not separate differentially expressed isoforms" << std::endl;
       ...
       stop_after_read == false & it != distance_thresholds.end(); ++it)
  {
    ...
    for (int t = 0; t < 2; t++)
    {
      ...
      if (file.good())
      {
        ...
        {
          std::cerr << "File already exists, " << filename << ". Use \"-f true\" option to overwrite " << std::endl;
          std::exit(1);
        }
      }
      file.close();
