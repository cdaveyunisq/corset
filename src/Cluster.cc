...
void Cluster::add_read(shared_ptr<Read> read) {
    read_.push_back(read);
};

shared_ptr<Transcript> Cluster::get_tran(int i) {
    return cluster_.at(i);
};

// lookup a transcript by na
shared_ptr<Transcript> Cluster::get_tran(string name) {
    return transcript_map[name];
}

shared_ptr<Read> Cluster::get_read(int i) {
    return read_.at(i);
};
...
void Cluster::merge(const int i, const int j) {
    ...
    for (int k = 0; k < i; k++) {
        if (dist.no_link(i, k) && ((k < j && dist.no_link(j, k)) || (j < k && dist.no_link(k, j)))) {
            continue;
        } else if ((dist.get(i, k) == UCHAR_MAX) && (
